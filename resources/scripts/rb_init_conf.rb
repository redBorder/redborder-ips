...
if opt["r"]
  ...
  if registration_mode == "proxy"
    ...
    system("echo 'Sensor #{ips_node_name} association in progress...' >> #{ConfigUtils.log_file}")
    system("/usr/lib/redborder/scripts/rb_associate_sensor.rb -u #{webui_user} -p #{webui_pass} -i #{Config_utils.get_ip_address} -m #{webui_host} >> #{ConfigUtils.log_file} 2>&1")
    if $?.exitstatus == 0
      ...
      puts "You can see logs in #{ConfigUtils.log_file}"
      system("/usr/lib/redborder/bin/rb_register_finish.sh >> #{ConfigUtils.log_file} 2>&1")
      ...
      puts "See \"#{ConfigUtils.log_file}\" for more details."
    end
  end
end
